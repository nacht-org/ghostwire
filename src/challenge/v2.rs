...
impl CloudflareV2 {
    ...
    pub fn extract_challenge_data(body: &str) -> Result<serde_json::Value> {
        let caps = RE_CF_OPT
            .captures(body)
            .ok_or_else(|| CloudscraperError::ChallengeError("Cannot find _cf_chl_opt".into()))?;
    ...
    pub fn extract_form_action(body: &str) -> Result<String> {
        let caps = RE_FORM_ACTION.captures(body).ok_or_else(|| {
            CloudscraperError::ChallengeError("Cannot find challenge form action".into())
        })?;
    ...
    pub fn build_js_payload(
        ...
        let r = RE_R_TOKEN
            .captures(body)
            .ok_or_else(|| CloudscraperError::ChallengeError("Cannot find r token".into()))?
            .get(1)
            .unwrap()
            .as_str()
            .to_string();
    ...
    pub fn extract_site_key(body: &str) -> Result<String> {
        let caps = RE_SITEKEY.captures(body).ok_or_else(|| {
            CloudscraperError::CaptchaError("Cannot find hCaptcha site key".into())
        })?;
        Ok(caps.get(1).unwrap().as_str().to_string())
    }
